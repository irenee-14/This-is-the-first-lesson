...
export default async function backgroundsRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Querystring: BackgroundListQuery }>(
    "/api/v1/backgrounds",
    {
      schema: {
        querystring: {
  ...
  fastify.get<{ Params: { backgroundId: string } }>(
  ...
  fastify.get<{ Querystring: { characterId: string } }>(
    "/api/v1/backgrounds/basic",
    {
      schema: {
        querystring: {
          type: "object",
  ...
  fastify.post<{ Body: CreateBackgroundRequest }>(
    "/api/v1/backgrounds",
    {
      schema: {
        body: {
        ...
        try {
          if (background.backgroundImg && character.characterImg) {
            ...
            if (artworkResult.imageUrl) {
              ...
              const relativePath = await downloadAndSaveImage(
  ...
  fastify.delete<{ Params: { backgroundId: string } }>(
    "/api/v1/backgrounds/:backgroundId",
    {
      schema: {
        params: {
