    ...
    async (request, reply) => {
      try {
        ...
        const openedSet = new Set<string>(
          openBackgrounds.map((ob: { backgroundId: string }) => ob.backgroundId)
        );
  ...
  fastify.get<{ Querystring: { writerId: string; characterId: string } }>(
    ...
    async (request, reply) => {
      try {
        ...
        const storyByBg = Object.fromEntries(
          stories.map((s) => [s.backgroundId, s] as const)
        );
        let seq = 1;
          ...
          if (isOpen && story && bg) {
            return {
              ...
              tags: Array.from(
                new Set([
                  ...(bg.tags as string[]),
                  ...(story.character.tags as string[]),
                ])
              ),
        ...
        // 캐릭터의 basic 스토리를 가장 앞에 추가
        const basicStoryCard = basicStory
          ? {
              id: basicStory.id,
              title: basicStory.name,
              imgUrl: basicStory.background.backgroundImg ?? null,
              tags: Array.from(
                new Set([
                  ...(basicStory.background.tags as string[]),
                  ...(basicStory.character.tags as string[]),
                ])
              ),
              description: basicStory.opening ?? null,
              dDay: null,
              isOpen: true, // basic 스토리는 항상 열려있음
            }
          : null;

        // basic 스토리가 있으면 가장 앞에 추가
        const finalCards = basicStoryCard ? [basicStoryCard, ...cards] : cards;

        return reply.send({ success: true, data: finalCards });
      } catch (error) {
        fastify.log.error(error);
        return reply.status(500).send({
          success: false,
